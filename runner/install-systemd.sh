...
#   APP_URL  AGENT_ID  RUNNER_DIR  PWSH  RUNNER_API_TOKEN  STALL_TIMEOUT  RUN_USER
set -euo pipefail
...
ExecStart=${PWSH} -NoProfile -ExecutionPolicy Bypass -File ${DIR}/Start-IamRunner.ps1 -AppUrl ${APP} -AgentId ${AGENT} -StallTimeoutSeconds ${STALL}
Restart=always
RestartSec=10
