// Shared page-data loader for /agents and /agents/v2 — queries, hover-hint assembly, and
...
export async function loadAgentsPage() {
  ...
  const vms: AgentVM[] = agents.map((a) => {
    ...
    return {
      ...
      browserInstallRequested: a.browserInstallRequested,
      browserInstallRequestedAt: a.browserInstallRequestedAt?.toISOString() ?? null,
      browserInstallRequestedBy: a.browserInstallRequestedBy ?? null,
      browserInstallDeliveredAt: a.browserInstallDeliveredAt?.toISOString() ?? null,
