// Zoom console API-credential auto-setup (the browser flow). The guided-setup modal's "Automatic
// (browser)" tab drives this:
//   POST → dispatch a full (signInOnly:false) Zoom console job: the runner signs in, creates the
//          "iam-engine" Server-to-Server OAuth app, and HARVESTS its Account ID / Client ID / Client
//          Secret, returning them in the job result. Returns { jobId }.
//   GET ?jobId= → poll to terminal. On success the harvested credential is VAULTED to Delinea here
//          (operator-authenticated: correct provenance + audit + Delinea write config) and the raw
//          values are SCRUBBED from the persisted job result immediately. Returns { done, ok, externalId }.
//
// The vault is inlined here (not the Mimecast branch's shared vault-module-credential.ts) so this PR is
// self-contained and conflict-free; a later refactor can unify them.
//
// SECURITY NOTE: the harvested Client Secret transits the job result until the first authenticated GET
...
import { dispatchZoomConsoleJob, ZOOM_CONSOLE_SECRET_NAME } from "@/lib/secrets/dispatch-zoom-console-job";
import { findHarvested, scrubHarvested } from "@/lib/secrets/zoom-harvest";
...
const ZOOM_SECRET = "zoom";
const norm = (s: string) => s.toLowerCase().replace(/\s+/g, "");

type ClientRow = { id: string; name: string; slug: string; delineaFolderId: string | null };

// Vault the harvested Zoom credential into Delinea, wire it, and record provenance. Inlined P0a vault
// (mirrors app/api/clients/[slug]/secrets/create/route.ts). Never logs values.
async function vaultZoom(client: ClientRow, values: Record<string, string>, setBy: string | null): Promise<{ ok: true; externalId: string; folderId: string } | { ok: false; error: string }> {
  const clientFolderId = folderIdFor(client.slug, client.delineaFolderId);
  const cap = delineaWriteConfigured({ slug: client.slug, secretName: ZOOM_SECRET, clientFolderId, allowTemplateByName: true });
  ...
  let tmpl = templateFor(ZOOM_SECRET);
  if (!tmpl) {
    const tmplName = defaultTemplateName(ZOOM_SECRET);
    const templateId = tmplName ? await findTemplateIdByName(cfg, tmplName, token) : null;
    if (templateId == null) return { ok: false, error: `no Secret Server template "${tmplName ?? ZOOM_SECRET}"` };
    tmpl = { templateId, fieldMap: defaultFieldMap(ZOOM_SECRET) };
  }
  const reqs = SECRET_FIELD_REQUIREMENTS[ZOOM_SECRET] ?? [];
  ...
  const moduleEntry = apiSetupBySecretName(ZOOM_SECRET);
  ...
  const ssName = `${client.name} — ${ZOOM_SECRET} (auto)`;
  ...
  await makeClientRepository(db).upsertSecrets(client.id, [{ name: ZOOM_SECRET, externalId, label: ssName }]);
  if (moduleEntry) {
...
export async function POST(req: Request, { params }: { params: { slug: string } }) {
  ...
  if (!consoleSecretRef) {
    const consoleSecret = await db.secret.findUnique({ where: { clientId_name: { clientId: client.id, name: ZOOM_CONSOLE_SECRET_NAME } }, select: { externalId: true } });
    if (!secretIsSet(consoleSecret?.externalId)) {
      return NextResponse.json({ error: "No Zoom console login is wired. Enter a Delinea secret ID, or wire a zoom-console secret (admin email + password) first.", needsConsoleSecret: true }, { status: 409 });
    }
  }

  const res = await dispatchZoomConsoleJob({ db, client, signInOnly: false, consoleSecretRef: consoleSecretRef || undefined });
  if (!res.ok) return NextResponse.json({ error: res.error }, { status: 502 });
  await recordAudit("zoom.console.create_app.dispatch", { user: g.user, clientId: client.id, jobId: res.jobId, detail: { usedSecretRef: Boolean(consoleSecretRef) } });
...
export async function GET(req: Request, { params }: { params: { slug: string } }) {
  ...
  if (alreadyScrubbed) {
    const wired = await db.secret.findUnique({ where: { clientId_name: { clientId: client.id, name: ZOOM_SECRET } }, select: { externalId: true } });
  ...
  if (!harvested) return NextResponse.json({ done: true, ok: false, error: "the setup completed but no Server-to-Server OAuth credential was harvested — re-run, or paste the credential manually." });

  const v = await vaultZoom(client, { "account id": harvested.accountId, "client id": harvested.clientId, "client secret": harvested.clientSecret }, auditActor(g.user, "ui").userId);
  // Scrub regardless of vault outcome — the raw secret must not linger.
  ...
  await recordAudit("zoom.console.create_app.vaulted", { user: g.user, clientId: client.id, jobId, detail: { externalId: v.externalId, folderId: v.folderId } });
