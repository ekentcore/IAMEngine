// POST /api/runner/google-ous/claim — { agentId }. The central (cloud) runner claims pending Google OU
// discoveries (FR #81); the response carries the brokered google-workspace credential fields. Never
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const work = await makeRunnerService(db).claimGoogleOuDiscovery(authed.id);
