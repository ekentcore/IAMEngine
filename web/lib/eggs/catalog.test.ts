...
test("live/idea splits cover the whole catalog", () => {
  ...
  // The shipped inventory: 25 live eggs — 14 from the first batches (specs 1-12 + day-of
  // greetings + pirate), the ten-egg batch, and the March 22 anniversary egg. If you ship or
  // add one, update the catalog AND these counts in the same PR.
  assert.equal(LIVE_EGGS.length, 25);
