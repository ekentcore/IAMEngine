...
test("onPremExclusions: legacy withholds nothing; a report withholds every unlisted on-prem key", () => {
  ...
  assert.deepEqual(onPremExclusions(["directory-sync"]), ["active-directory", "ad-email-writeback", "ad-consistency-check", "ad-hard-match", "ad-password-reset", "ad-remove-user", "ad-correct-user"]);
});
